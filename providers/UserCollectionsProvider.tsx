...
interface UserCollectionsContextReturn extends ReturnType<typeof useCollections> {
  isLoading: boolean;
}
...
const UserCollectionsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <UserCollectionsContext.Provider value={value}>{children}</UserCollectionsContext.Provider>
  );
};
