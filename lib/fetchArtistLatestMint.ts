...
const getArtistLatestMint = async (
  ...
  try {
    ...
    if (!data.tokenContract || !data.tokenId) throw new Error("no moments yet.");
    const metadata = await fetchTokenMetadata(data.tokenContract, BigInt(data.tokenId).toString());
    return metadata;
  } catch (error) {
    console.error(error);
    return null;
