...
const getSwapMintComments = async (tokenContract: string): Promise<DuneDecodedEvent[]> => {
  const options = {
    method: "GET",
  ...
  while (1) {
    try {
      ...
      if (data.transactions.length) transactions = transactions.concat(data.transactions);
