...
const useGrantMomentPermission = () => {
  ...
  const grantPermission = async () => {
    if (!moment?.collectionAddress || !moment?.tokenId) {
      toast.error("Moment data not available");
      return;
    }
    ...
    try {
      ...
      // Write the contract transaction to add permission at moment level (using actual tokenId)
      const hash = await client.writeContract({
        address: moment.collectionAddress as Address,
        abi: zoraCreator1155ImplABI,
        functionName: "addPermission",
        args: [BigInt(moment.tokenId), smartWallet as Address, BigInt(PERMISSION_BIT_ADMIN)],
      });
...
export default useGrantMomentPermission;
